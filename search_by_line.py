...
def gtfs_view():
    ...
    selected_valik = st.selectbox("Sisesta või vali liini number", [""] + list(route_map.keys()))
    if selected_valik:
        selected_route_id = route_map[selected_valik]
        st.markdown(f"### Valitud liin: **{selected_valik}**  \nRoute ID: `{selected_route_id}`")

        filtered_trips = trips_df[trips_df['route_id'] == selected_route_id]
        if not filtered_trips.empty:
            st.write(f"Seotud trips liiniga {selected_valik}:")
